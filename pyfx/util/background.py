...
class Background:
    ...
    def __init__(self,bg_file,blur_sigma=10):
        ...
        self._bg_img = util.load_image_file(bg_file,return_array=False)

        self._bg_array_color = util.image_to_array(self._bg_img,num_channels=3,dtype=np.float)
        self._bg_array_bw = util.image_to_array(self._bg_img,num_channels=1,dtype=np.float)
        self._bg_array_blur = filters.gaussian(self._bg_array_bw,self._blur_sigma)
        self._bg_out = util.image_to_array(self._bg_img,num_channels=4,dtype=np.uint8)

    def get_frame_diff(self,img_file):
        """
        """

        img_array_bw = util.load_image_file(img_file,dtype=np.float,num_channels=1)
